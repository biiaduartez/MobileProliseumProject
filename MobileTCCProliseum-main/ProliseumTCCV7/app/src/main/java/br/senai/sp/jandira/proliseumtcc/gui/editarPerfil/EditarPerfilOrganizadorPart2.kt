...
@Composable
fun EditarPerfilOrganizadorPart2(rememberNavController: NavController) {

    val customFontFamilyTitle = FontFamily(Font(R.font.font_title))
    val customFontFamilyText = FontFamily(Font(R.font.font_poppins))

    var editarFotoPerfilOrganizadorUri by remember {
        mutableStateOf<Uri?>(null)
    }

    var launcherEditarFotoPerfilOrganizador = rememberLauncherForActivityResult(
        contract = ActivityResultContracts.GetContent()
    ){
        editarFotoPerfilOrganizadorUri = it
    }

    var painterFotoPerfilOrganizador = rememberAsyncImagePainter(
        ImageRequest.Builder(LocalContext.current)
            .data(editarFotoPerfilOrganizadorUri)
            .build()
    )
    ...
    Box(
        ...
        Column(
            ...
            Row(
                ...
                Icon(
                    modifier = Modifier.clickable { rememberNavController.navigate("editar_perfil_organizador_part_1") },
                        ...
                        Box(contentAlignment = Alignment.BottomEnd){
                            Card(
                                modifier = Modifier
                                    .size(100.dp)
                                    .clickable {
                                        launcherEditarFotoPerfilOrganizador.launch("image/*")
                                        var message = "nada"
                                        Log.i(
                                            "PROLISEUM",
                                            "URI: ${editarFotoPerfilOrganizadorUri?.path ?: message} "
                                        )
                                    },
                                shape = CircleShape
                            ){
                                Image(
                                    ...
                                    painter = if(editarFotoPerfilOrganizadorUri == null) painterResource (id = R.drawable.superpersonicon) else painterFotoPerfilOrganizador,
                                    contentDescription = "",
...
@Composable
fun EditarPerfilOrganizadorPart2ScreenPreview() {
    ProliseumTCCTheme{
